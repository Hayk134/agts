# Модуль АСО АНТС (Профессионалы 2026)
...
```shell
# Инициализация системы (при первом запуске) в директории с АСО АНТС 
uv sync
```
...
Исключением являются случаи, согласованные с ГЭ в процессе выполнения задач соревнования.
